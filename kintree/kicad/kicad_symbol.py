...
class ComponentLibManager(object):
    def __init__(self, library_path):
        ...
        cprint('[KCAD]\tNumber of parts in library ' + library_name + ': ' + str(len(self.kicad_lib.symbols)), silent=settings.SILENT)

    def is_component_in_library(self, component_part_number):
        ''' Check if component already exists in library '''
        for component in self.kicad_lib.symbols:
            cprint(f'[DBUG]\t{component.name} ?= {component_part_number}', silent=settings.HIDE_DEBUG)
            if component.name == component_part_number:
                cprint(f'[KCAD]\tWarning: Component {component_part_number} already in library', silent=settings.SILENT)
                return True

        return False

    def add_component_to_library_from_inventree(self, component_data, template_path=None, show_progress=True):
        ...
        try:
            is_component_in_library = self.is_component_in_library(component_data['IPN'])
            part_in_lib = True
        except:
        ...
        # Load template
        templatelib = kicad_sym.KicadLibrary(template_path)
        # Load new component
    ...
    #     kicad_lib = kicad_sym.KicadLibrary(library_path)
    #     library_name = library_path.split(os.sep)[-1]
    #     cprint('[KCAD]\tNumber of parts in library ' + library_name + ': ' + str(kicad_lib.get_component_count()), silent=settings.SILENT)

    #     try:
    #         kicad_lib.delete_component(part_number)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2:
        ...
        if not kicad_lib.is_component_in_library(mpn):
